...
func _populate_pool(size: int) -> void:
	for i in range(size):
		var object: Packet = GlobalData.PACKET_SCENE.instantiate()
...
func get_object( pkt_speed: int, pkt_path: Array[Building], pkt_position: Vector2) -> Packet:
	...
	var object: Packet = object_pool.pop_front()
	...
	object.speed = pkt_speed
	object.path = pkt_path
	object.global_position = pkt_position
...
func return_object(object: Packet) -> void:
